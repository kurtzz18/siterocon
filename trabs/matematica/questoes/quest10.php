...
<body>
    ...
    <main>
        ...
        <?php
            $sql=('select * from questoes where quiz_questao="10"');
        ...
        <section>
            <div id="conteudo">
                <h1 id="questao_titulo">10ª Questão </h1><br>
                ...
                <?php
                    if(isset($_POST['resp1'])){
                        $resp=$_POST['resp1'];
                        if($resp=="1"){
                            $pontuacao=$_SESSION['score_rascunho']+10;
                            $sql=('update usuarios set pontuacao='.$pontuacao.' where nome="'.$_SESSION['usuario'].'";');
                            $sql_mostrar=mysqli_query($conexao, $sql);
                            header('location:../rank.php');
                        } else {
                            $pontuacao=$_SESSION['score_rascunho'];
                            $sql=('update usuarios set pontuacao='.$pontuacao.' where nome="'.$_SESSION['usuario'].'";');
                            $sql_mostrar=mysqli_query($conexao, $sql);
                            header('location:../rank.php');
                        }
                    ...
                    if(isset($_POST['resp2'])){
                        ...
                        if($resp=="2"){
                            $pontuacao=$_SESSION['score_rascunho']+10;
                            $sql=('update usuarios set pontuacao='.$pontuacao.' where login="'.$_SESSION['usuario'].'";');
                            $sql_mostrar=mysqli_query($conexao, $sql);
                            header('location:../rank.php');
                        } else {
                            $pontuacao=$_SESSION['score_rascunho'];
